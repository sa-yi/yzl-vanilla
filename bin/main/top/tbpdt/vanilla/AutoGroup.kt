...
import top.tbpdt.configer.AutoConfig.defaultCallLingMessage
import top.tbpdt.configer.AutoConfig.defaultGoodMessage
...
import top.tbpdt.configer.GlobalConfig.callLingImgDir
...
//    }
    ...
    @EventHandler(priority = EventPriority.LOWEST)
    suspend fun GroupMessageEvent.help() {
        if (message.isCommand("help")) {
            subject.sendMessage("说明书：bot.tbpdt.top\n" +
                    "GitHub：@YZLbot\n" +
                    "饲养群：758502512\n" +
                    "主人：1492515593")
        }
    ...
    @EventHandler(priority = EventPriority.LOWEST)
    suspend fun GroupMessageEvent.calledLing() {
        val plainText = message.getPlainText().trim()
        if (!plainText.contains("阿绫")) return

        val randomNumber = Random.nextDouble()

        // 根据随机数决定执行哪个流程
        if (randomNumber < 0.7) {
            val memeDir = callLingImgDir
            val files = File(memeDir).listFiles { _, name -> name.matches(Regex(".*\\.(jpg|jpeg|png|gif)$")) }

            if (files != null && files.isNotEmpty()) {
                val randomIndex = Random.nextInt(files.size)
                val image = files[randomIndex]
                group.sendImage(image)
                return
            }
        }else {
            val randomResponse = defaultCallLingMessage.random()
            group.sendMessage(randomResponse)
        }
    }


    @EventHandler(priority = EventPriority.LOWEST)
    suspend fun GroupMessageEvent.good() {
        if (sender.id !in GlobalConfig.admin) return
        val plainText = message.getPlainText().trim()
        if (plainText != "好好好") return
        val msg= defaultGoodMessage.random()
